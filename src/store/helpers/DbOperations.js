...
class DbOperations {
	...
	// 	})
	// };
	loadFilteredDriversData(filterName, expFrom, expTo) {
		const queryFilters = [];

		if (filterName) {
			queryFilters.push(where('name', '>=', filterName));
		}
		if (expFrom) {
			queryFilters.push(where('experience', '>=', expFrom));
		}
		if (expTo) {
			queryFilters.push(where('experience', '<=', expTo));
		}
		const q = query(this.dbCollection, ...queryFilters);
